...
class Buffer1(SerializedBuffer):
...
class RolloutBuffer1:
    ...
    def append(self, state, action, reward, done, log_pi, next_state):
        self.states[self._m].copy_(torch.from_numpy(state))
        self.actions[self._m].copy_(torch.from_numpy(action))
        self.rewards[self._m] = float(reward)
        self.dones[self._m] = float(done)
        self.log_pis[self._m] = float(log_pi)
        self.next_states[self._m].copy_(torch.from_numpy(next_state))

        self._m = (self._m + 1) % self.total_size
        self._n = min(self._n + 1, self.total_size)

    def get(self):

        start = (self._m - self.buffer_size) % self.total_size
        idxes = slice(start, start + self.buffer_size)
        return (
            self.states[idxes],
            self.actions[idxes],
