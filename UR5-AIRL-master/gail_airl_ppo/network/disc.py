...
class AIRLDiscrim(nn.Module):
    ...
    def D(self, states, dones, next_states, log_pis):
        f = self.f(states, dones, next_states)
        D = torch.exp(f)/(torch.exp(f)+log_pis)
        return D
    ...
    def calculate_reward(self, states, actions, dones, log_pis, next_states):
        # with torch.no_grad():
        #     D = self.D(states, dones, next_states, log_pis).detach()
