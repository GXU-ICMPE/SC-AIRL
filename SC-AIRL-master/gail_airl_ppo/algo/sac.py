...
from gail_airl_ppo.network import (
    StateDependentPolicy, TwinnedStateActionFunction, StateIndependentPolicy
)
...
class SAC(Algorithm):
    ...
    def __init__(self, state_shape, action_shape, device, seed, gamma=0.99,
                 batch_size=512, buffer_size=10**6, lr_actor=3e-4,
                 lr_critic=3e-4, lr_alpha=3e-4, units_actor=(256, 256),
                 units_critic=(256, 256), start_steps=2048, tau=5e-3):
        super().__init__(state_shape, action_shape, device, seed, gamma)  # path
...
class SACExpert2(SAC):

    def __init__(self, state_shape, action_shape, device, path0, path1, path2, path3,
                 units_actor=(256, 256)):
        self.actor0 = StateDependentPolicy(
            state_shape=state_shape,
            action_shape=action_shape,
        ...
        self.actor3 = StateDependentPolicy(
            state_shape=state_shape,
            action_shape=action_shape,
            hidden_units=(256, 256),
            hidden_activation=nn.ReLU(inplace=True)
...
class SACExpert1(SAC):
    ...
    def __init__(self, state_shape, action_shape, device, path0, path1, path2, path3,
        ...
        self.actor1 = StateDependentPolicy(
            state_shape=state_shape,
            action_shape=action_shape,
            hidden_units=(256, 256),
            hidden_activation=nn.ReLU(inplace=True)
        ...
        self.actor2 = StateDependentPolicy(
            state_shape=state_shape,
            action_shape=action_shape,
            hidden_units=(256, 256),
            hidden_activation=nn.ReLU(inplace=True)
